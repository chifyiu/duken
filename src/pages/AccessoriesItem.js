...
const AccessoriesItem = ({ accessories, addToCart, secondImage }) => {
  ...
  useEffect(() => {
    const storedReviews = JSON.parse(localStorage.getItem(`reviews-${accessories.id}`)) || [];
  ...
  const handleReviewSubmit = (e) => {
    ...
    localStorage.setItem(`reviews-${accessories.id}`, JSON.stringify(updatedReviews));
    setName('');
    setReviewText('');
    setRating(0);
    setPhoto(null);
    fileInputRef.current.value = ''; // Clear the file input
    calculateAverageRating(updatedReviews); // Пересчитываем средний рейтинг после добавления нового отзыва
  };
  ...
  return (
    <div className="catalog-container">
      <div className="movie-card">
        <div className="clothing-image" style={{ backgroundImage: `url(${accessories.image})` }}></div>
        ...
        <div className="clothing-info">
          ...
          <div className="cloth-description">
            ...
            {showCare && <div className="description-box">{accessories.care}</div>}
            
            <button onClick={() => setShowOrigin(!showOrigin)}>Происхождение</button>
            {showOrigin && <div className="description-box">{accessories.origin}</div>}
          </div>
