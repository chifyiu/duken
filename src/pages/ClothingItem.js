...
const ClothingItem = ({ clothing, addToCart, secondImage }) => {
  ...
  useEffect(() => {
    const storedReviews = JSON.parse(localStorage.getItem(`reviews-${clothing.id}`)) || [];
  ...
  const handleReviewSubmit = (e) => {
    ...
    localStorage.setItem(`reviews-${clothing.id}`, JSON.stringify(updatedReviews));
    setName('');
    setReviewText('');
    setRating(0);
    setPhoto(null);
  ...
  const addToCartHandler = () => {
    ...
    addToCart(clothing, selectedSize);
  ...
  return (
    <div className="catalog-container">
      <div className="movie-card">
        <div className="clothing-image" style={{ backgroundImage: `url(${clothing.image})` }}></div>
        ...
        <div className="clothing-info">
          ...
          <div className="cloth-description">
            ...
            {showCare && <div className="description-box">{clothing.care}</div>}
            
            <button onClick={() => setShowOrigin(!showOrigin)}>Происхождение</button>
            {showOrigin && <div className="description-box">{clothing.origin}</div>}
          </div>
